...
import { BookOpen, Plus, Trash2, GripVertical, Sparkles, Play, Loader2, ChevronDown, ChevronUp, Download, Settings2, Users, Link } from "lucide-react";
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (apiScenes.length > 0) {
          ...
          // Auto-generate visual anchor from concept
          if (!visualAnchor) {
    ...
    try {
      ...
      pollRef.current = setInterval(async () => {
        try {
          ...
          // API returns progress as 0-100, NOT 0-1
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="space-y-4">
          ...
          <Card>
            ...
            <Button className="w-full mt-3" onClick={handleGenerateScript} disabled={!concept.trim() || scriptLoading}>
              ...
              {continuityMode !== "none" && (
                <div>
                  ...
                  <textarea value={visualAnchor} onChange={(e) => setVisualAnchor(e.target.value)}
                    placeholder="Describe consistent visual elements: main subject appearance, color palette, camera style..."
                    className="w-full h-16 bg-zinc-800/50 border border-zinc-700 rounded-lg px-2.5 py-2 text-[11px] text-zinc-200 placeholder-zinc-600 focus:ring-2 focus:ring-indigo-500/50 resize-none mt-0.5" />
                  <p className="text-[9px] text-zinc-600 mt-0.5">This gets appended to every scene prompt for consistent subjects/style</p>
                </div>
              )}
            </div>
          </Card>
        ...
        <div className="lg:col-span-2 space-y-4">
          ...
          {scenes.length === 0 && !scriptLoading && (
            <Card className="min-h-[200px] flex items-center justify-center">
              <div className="text-center">
                ...
                <p className="text-zinc-600 text-xs mt-1">Pick a story idea and click "AI Generate Script"</p>
              </div>
            </Card>
          )}
            ...
            <>
              <Button size="lg" className="w-full" onClick={handleGenerateMovie} disabled={generating}>
                {generating ? (
                  ...
                  <><Play className="w-5 h-5 mr-2" /> Generate Movie ({scenes.length} scenes, {resolution}, {fps}fps)</>
                )}
              </Button>
              {generating && (
                <Card>
          ...
          {resultJobId && (
            <Card className="border-green-500/20 bg-green-500/5">
              ...
              <video src={getDownloadUrl(resultJobId)} controls autoPlay loop muted className="w-full rounded-lg border border-zinc-700" />
              <a href={getDownloadUrl(resultJobId)} download className="block mt-3">
                <Button variant="secondary" className="w-full"><Download className="w-4 h-4 mr-2" /> Download Movie</Button>
              </a>
            </Card>
          )}
